...
function VideoTitle({title, overview}) {
    ...
    return (
        <div className='text-white bg-gradient-to-tr  from-black w-full h-screen absolute pt-[35%] pl-18'>
            <h1 className={`transition-all text-5xl font-extrabold mb-4 duration-1000 ${showMiddle ? "translate-y-0" : "translate-y-16"
        }`}>{title}</h1>
            <p className='w-1/3 opacity-100 mb-4 animate-[fadeout_10s_forwards]'>{overview}</p>
            <div className='flex space-x-4 '>
                <button className='px-6 py-2 flex items-center font-semibold text-md text-center bg-white hover:bg-gray-200/80 duration-200 text-black rounded-sm cursor-pointer'>
